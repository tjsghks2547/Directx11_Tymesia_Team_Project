...
void CBone::Update_CombinedTransformationMatrix(const vector<class CBone*>& Bones, _fmatrix PreTransformMatrix, _float fTimeDelta)
{
	...
	if (m_iParentBoneIndex == 1)
	{
		...
		// 이전 프레임의 루트 본 위치와 현재 프레임의 루트 본 위치의 차이를 계산하여 프레임 간 이동량을 계산.
		matrix.r[3] = XMVectorSetW(matrix.r[3], 1.f); // 이동 변화량을 적용 시켜야 하므로 W 값을 1로 고정		


		// 결과를 저장
		XMStoreFloat4x4(&m_RootMotionMatrix, matrix);  // 해당 이동량을 저장 
		XMStoreFloat4x4(&m_pPreRootMatrix, XMLoadFloat4x4(&m_CombinedTransformationMatrix));


		m_CombinedTransformationMatrix._41 = 0.f; // Combined Matrix의 x값 초기화
		m_CombinedTransformationMatrix._42 = 0.f; // Combined Matrix의 y값 초기화
		m_CombinedTransformationMatrix._43 = 0.f; // Combined Matrix의 z값 초기화
		m_CombinedTransformationMatrix._44 = 1.f; // Combined Matrix의 w값 1 초기화
