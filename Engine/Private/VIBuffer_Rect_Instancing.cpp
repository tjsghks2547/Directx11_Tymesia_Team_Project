...
HRESULT CVIBuffer_Rect_Instancing::Initialize_Prototype(const PARTICLE_INSTANCING_DESC* pArg)
{
...
{
    /* 사본을 만들때 실제 활용하기 위한 인스턴스 정점 버퍼를 할당한다. */
...
CVIBuffer_Rect_Instancing* CVIBuffer_Rect_Instancing::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const PARTICLE_INSTANCING_DESC* pArg)
{
    CVIBuffer_Rect_Instancing* pInstance = new CVIBuffer_Rect_Instancing(pDevice, pContext);   

    if(FAILED(pInstance->Initialize_Prototype(pArg)))
    {
        MSG_BOX("Failed to Created : VIBuffer_Rect_Instancing");
...
CComponent* CVIBuffer_Rect_Instancing::Clone(void* pArg)
{
    ...
    {
        MSG_BOX("Failed to Cloned : VIBuffer_Rect_Instancing");
        Safe_Release(pInstance);
    }
