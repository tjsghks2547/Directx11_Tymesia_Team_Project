...
CBody_VillageF1::CBody_VillageF1(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	:CPartObject(pDevice, pContext)
{
}

CBody_VillageF1::CBody_VillageF1(const CBody_VillageF1& Prototype)
	:CPartObject(Prototype)
{
}

HRESULT CBody_VillageF1::Initialize_Prototype()
{
...
void CBody_VillageF1::Update(_float fTimeDelta)
{
...
HRESULT CBody_VillageF1::Render_Shadow()
{
	return S_OK;
}

HRESULT CBody_VillageF1::Ready_Components()
{
	/* Com_Shader */
	...
	/* Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_Normal_VillageF1_Body"),
...
HRESULT CBody_VillageF1::Bind_ShaderResources()
{
...
CBody_VillageF1* CBody_VillageF1::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBody_VillageF1* pInstance = new CBody_VillageF1(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CBody_VillageF1");
...
CGameObject* CBody_VillageF1::Clone(void* pArg)
{
	CBody_VillageF1* pInstance = new CBody_VillageF1(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CBody_VillageF1");
		Safe_Release(pInstance);
	}
