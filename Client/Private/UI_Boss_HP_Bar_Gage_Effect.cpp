...
CUI_Boss_HP_Bar_Gage_Effect::CUI_Boss_HP_Bar_Gage_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    :CUI_Image(pDevice, pContext)
{
}

CUI_Boss_HP_Bar_Gage_Effect::CUI_Boss_HP_Bar_Gage_Effect(const CUI_Boss_HP_Bar_Gage_Effect& Prototype)
    :CUI_Image(Prototype)
{
}

HRESULT CUI_Boss_HP_Bar_Gage_Effect::Initialize_Prototype()
{
...
void CUI_Boss_HP_Bar_Gage_Effect::Priority_Update(_float fTimeDelta)
{
    ...
    if (m_bActive)
    {
        ...
        m_pTransformCom->Scaling(_float3(m_fSizeX, m_fSizeY * (1 + (m_fTimeDelta * 2.f)), 1.f));
    }
...
void CUI_Boss_HP_Bar_Gage_Effect::Update(_float fTimeDelta)
{
    ...
    if (m_fTimeDelta >= 1.f)
    {
        m_iTest = 0;
        m_fTimeDelta = 0.f;
        m_fBoss_Pre_HP = *m_fBoss_Cur_HP;
        m_fBoss_Pre_Shield_HP = *m_fBoss_Shield_HP;
        m_bActive = false;
    }
}
...
HRESULT CUI_Boss_HP_Bar_Gage_Effect::Render()
{
    ...
    for (_uint i = 1; i <= 2; i++)
    {
        ...
        if (i == 1)
        {
            ...
            pPreHP = m_fBoss_Pre_HP / *m_fBoss_Max_HP;
        }
        else
        {
            pCurHP = *m_fBoss_Shield_HP / *m_fBoss_Max_HP;
            pPreHP = m_fBoss_Pre_Shield_HP / *m_fBoss_Max_HP;
        }
...
HRESULT CUI_Boss_HP_Bar_Gage_Effect::Ready_Components()
{
    /* Com_Texture */
...
CUI_Boss_HP_Bar_Gage_Effect* CUI_Boss_HP_Bar_Gage_Effect::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CUI_Boss_HP_Bar_Gage_Effect* pInstance = new CUI_Boss_HP_Bar_Gage_Effect(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed To Created : CUI_Boss_HP_Bar_Gage_Effect");
...
CGameObject* CUI_Boss_HP_Bar_Gage_Effect::Clone(void* pArg)
{
    CUI_Boss_HP_Bar_Gage_Effect* pInstance = new CUI_Boss_HP_Bar_Gage_Effect(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed To Cloned : CUI_Boss_HP_Bar_Gage_Effect");
        Safe_Release(pInstance);
    }
...
void CUI_Boss_HP_Bar_Gage_Effect::Free()
{
    __super::Free();
